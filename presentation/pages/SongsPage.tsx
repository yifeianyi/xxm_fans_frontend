...
const SongsPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'hot' | 'all' | 'originals'>('all');

  const getTitle = () => {
    switch (activeTab) {
      case 'hot': return '咻咻满热歌榜';
      case 'originals': return '咻咻满原创作品';
  ...
  const getPageTitle = () => {
    switch (activeTab) {
      ...
      case 'originals': return '咻咻满原创作品 - 个人原创单曲';
    ...
    <>
      ...
      <div className="w-full max-w-xl mx-auto">
        <div className="relative flex p-1.5 bg-white/40 rounded-full shadow-inner border-2 border-white overflow-hidden">
          <div
            className={`absolute top-1.5 bottom-1.5 w-[calc(33.333%-6px)] bg-gradient-to-r from-[#f8b195] to-[#f67280] rounded-full transition-all duration-500 ease-out z-0`}
            style={{
              left: activeTab === 'hot' ? '6px' : activeTab === 'all' ? 'calc(33.333% - 2px)' : 'calc(66.666% - 2px)'
            }}
          ></div>
