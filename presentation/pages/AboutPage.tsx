...
const AboutPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [settingsResult, milestonesResult] = await Promise.all([
                    siteSettingsService.getSiteSettings(),
                    siteSettingsService.getMilestones(),
                ]);
    ...
    const bioInfo = [
        ...
        { icon: <Coffee size={18} />, label: '职业', value: siteSettings?.artist_profession?.join('、') || '歌手、配音演员、唱见' },
    ];
