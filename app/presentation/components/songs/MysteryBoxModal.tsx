...
export default function MysteryBoxModal({ 
    ...
    return (
        <div 
            className="fixed inset-0 z-[9998] flex items-center justify-center p-4 bg-[#4a3728]/30 backdrop-blur-xl animate-in fade-in duration-500"
            onClick={onClose}
        >
            <div 
                className="relative w-full max-w-4xl bg-white rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 border-4 border-white"
                onClick={e => e.stopPropagation()}
            >
                {/* 头部 */}
                <div className="px-8 py-6 flex items-center justify-between border-b border-[#f8b195]/10">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-[#f8b195] text-white rounded-2xl flex items-center justify-center shadow-lg">
                            <Sparkles size={20} />
                        </div>
